...
#compare followed with not-followed
```{r}
data <- read.csv("data_completed.csv", 
                    stringsAsFactors = FALSE)

# Extract the vector of followed names
followed <- unique(df$case_id)

data.unf <- data %>%
  filter(!(case_id %in% followed))

data.unf <- data.unf %>%
  mutate(ICU = if_else(ICU == "yes", 1, 0))
...
data.fol$clin_score <- data.fol$number_symptoms_admission


#Age, sex, Treatment (also IVIG vs. IVIG + Steroide), Intensive Care treatment, Myokardial dysfunction, Coronary artery aneurysm, ggf. Laborwerte zumindest CRP

continuous <- c('clin_score', 'plab_crp_si_value', 'pther_inpatient_days', "ppatient_age_years")
categorical <- c("ICU", "pther_immun_mod_type___13", "pther_immun_mod_type___2", "pimg_echo_myocard_yesno", 
                 "pimg_echo_coronpath_yesno", "ppatient_gender")

continuous[continuous %in% colnames(data.unf)]
categorical[categorical %in% colnames(data.fol)]


# 1. CATEGORICAL COMPARISON (Chi-square or Fisher's)
cat_summary <- purrr::map_dfr(categorical, function(var) {
  if (!(var %in% colnames(data.fol)) | !(var %in% colnames(data.unf))) {
    warning(paste("Variable", var, "not found in both datasets. Skipping."))
    return(NULL)
  }

  fol_var <- data.fol[[var]]
  unf_var <- data.unf[[var]]
  
  # Remove NAs
  fol_var <- fol_var[!is.na(fol_var)]
  unf_var <- unf_var[!is.na(unf_var)]

  tab <- table(factor(c(fol_var, unf_var)), 
               group = rep(c("followed", "unfollowed"), 
                           c(length(fol_var), length(unf_var))))
  
  if (ncol(tab) < 2) {
    warning(paste("Variable", var, "does not have both groups. Skipping."))
    return(NULL)
  }

  test <- if (any(tab < 5)) fisher.test(tab) else chisq.test(tab)

  purrr::map_dfr(rownames(tab), function(level) {
    followed_count <- tab[level, "followed"]
    followed_total <- sum(tab[, "followed"])
    unfollowed_count <- tab[level, "unfollowed"]
    unfollowed_total <- sum(tab[, "unfollowed"])
    ...
    tibble(
      ...
      Followed = sprintf(
        "%d/%d (%.1f%%)",
        followed_count, followed_total, 100 * followed_count / followed_total
      ),
      Not_followed = sprintf(
        "%d/%d (%.1f%%)",
        unfollowed_count, unfollowed_total, 100 * unfollowed_count / unfollowed_total
      ),
...
# 2. CONTINUOUS COMPARISON (Wilcoxon test, median/IQR-range)
cont_summary <- purrr::map_dfr(continuous, function(var) {
  if (!(var %in% colnames(data.fol)) | !(var %in% colnames(data.unf))) {
    warning(paste("Variable", var, "not found in both datasets. Skipping."))
    return(NULL)
  }

  fol_var <- data.fol[[var]] %>% na.omit()
  unf_var <- data.unf[[var]] %>% na.omit()

  if (length(fol_var) == 0 | length(unf_var) == 0) {
    return(tibble(variable = var, level = NA, Followed = NA, Not_followed = NA, p_value = NA))
  }

  # Wilcoxon test
  test <- wilcox.test(fol_var, unf_var)

  tibble(
    variable = var,
    level = NA,
    Followed = sprintf("%.1f (%.1f–%.1f)", median(fol_var), IQR(fol_var), max(fol_var) - min(fol_var)),
    Not_followed = sprintf("%.1f (%.1f–%.1f)", median(unf_var), IQR(unf_var), max(unf_var) - min(unf_var)),
    p_value = signif(test$p.value, 3)
  )
})
...
final_table <- bind_rows(cat_summary, cont_summary) %>%
  mutate(
    p_adj = p.adjust(p_value, method = "BH"),
    p_adj = ifelse(p_adj < 0.001, "<0.001", sprintf("%.3f", p_adj))
  ) %>%
  select(variable, level, Followed, Not_followed, p_adj)

# 4. Optional filtering
filtered_final_table <- final_table %>%
  slice(-c(1, 3, 5, 7, 8, 10, 11, 13, 14)) %>%
  filter(!variable %in% c("p_value", "level")) %>%
  select(-level)  # <- remove the column explicitly



filtered_final_table <- filtered_final_table %>%
  rename(
    Variable = variable,
    'Not followed' = Not_followed
  )

name_map <- c('clin_score'="N° of affected organ systems", 
              'plab_crp_si_value' = "CRP (mg/l)", 
              'pther_inpatient_days' = "Days of hospitalisation", 
              "ppatient_age_years" = "Patient age (years)", 
              "ICU" = "ICU",
              "pther_immun_mod_type___13" = "IVIGs", 
              "pther_immun_mod_type___2" = "Corticosteroids", 
              "pimg_echo_myocard_yesno" ="Myocardial dysfunction", 
              "pimg_echo_coronpath_yesno" = "Coronary dilatation", 
              "ppatient_gender" = "Patient gender (female)")

filtered_final_table <- filtered_final_table %>%
  mutate(Variable = ifelse(Variable %in% names(name_map), name_map[Variable], Variable))

# Show and print result
ft <- flextable(filtered_final_table)

doc <- read_docx() %>%
  body_add_flextable(ft)

print(doc, target = "table_comparison.docx")
```
...
ggsave(paste0(dir, "/bar_Number of symptoms.png"), dpi = 600, width = 5, height = 4)

```


#spaghetti plot on recovey
```{r}
# 1) Summarise per patient, then per group
summary_df <- df %>%
  group_by(case_id) %>%
  arrange(Time) %>%
  reframe(
    baseline_symptoms = first(number_symptoms),
    time_to_zero = first(Time[number_symptoms == 0])
  ) %>%
  summarize(
    median_baseline     = median(baseline_symptoms, na.rm = TRUE),
    median_time_to_zero = median(time_to_zero,     na.rm = TRUE),
    .groups = "drop"
  )
head(summary_df)

ggplot(df, aes(x = Time, y = number_symptoms, group = case_id)) +
  geom_line(color = "cornflowerblue", linewidth = 0.7, alpha = 0.2) +
  ...
  geom_segment(
    ...
    aes(x = 0,
        y = median_baseline,
        xend = median_time_to_zero,
        yend = 0),
    color = "darkblue",  # different color for median segment
    linewidth = 1.5
  ) +
  ...
  labs(
    x     = "Days",
    y     = "N° of affected organ systems",
    title = "Individual recovery trajectories"
  ) +
  theme_minimal() +
  theme(
...
ggsave(paste0(dir, "/spaghetti_recovery.png"), dpi = 600, width = 5, height = 4)
```
